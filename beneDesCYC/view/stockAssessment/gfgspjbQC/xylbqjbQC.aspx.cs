...
namespace beneDesCYC.view.stockAssessment.gfgspjbQC
{
    public partial class xylbqjbQC : beneDesYGS.core.UI.corePage
    {
        ...
        protected void QS_Click()
        {
            ...
            string Path = Server.MapPath("beneDesCYC/gfgspjbQC.xml");
            xmlDoc.Load(Path);
        ...
        protected void isSelected()
        {
            ...
            selectDs.ReadXml(Server.MapPath("/beneDesCYC/gfgspjbQC.xml"));
        ...
        protected void DC_Click(object sender, EventArgs e)
        {
            ...
            this.FpSpread2.Sheets[0].OpenExcel(path, "表35-气井效益类别全表");
                ...
                FpSpread2.Sheets[0].RowHeader.Visible = false;
                    ...
                    FpSpread2.Sheets[0].AddColumns(hcount, ccount - 3);
                    //动态生成表头                  
        ...
        protected void Fp_DataBound(int page)
        {
            ...
            try
            {
                ...
                //从XML文件中读取已勾选的要查询字段
                DataSet selectDs = new DataSet();
                selectDs.ReadXml(Server.MapPath("gfgspjbQC.xml"));
                DataTable selectDt = selectDs.Tables[0];
                for (int i = 0; i < selectDt.Rows.Count; i++)
                {
                    if (selectDt.Rows[i]["checked"].ToString() == "1")
                ...
                string fql = "select {0} from jdstat_djsj sdy,gsxylb_info xy where sdy.djisopen='1' and sdy.cyc_id= '{1}' and sdy.gsxyjb=xy.xyjb order by gsxyjb ";
                fql = string.Format(fql, selectItems, Session["cyc_id"].ToString());
