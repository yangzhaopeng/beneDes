...
namespace beneDesCYC.core
{
    ...
    public class SqlHelper
    {
        ...
        {
            string strconn = ConfigurationManager.ConnectionStrings["cycConnectionString"].ConnectionString;
            OracleConnection conn = new OracleConnection(strconn);
            //string Strconn = ConfigurationSettings.AppSettings["conn"].ToString();
            //OracleConnection conn = new OracleConnection(Strconn);

            //OracleConnection conn = new OracleConnection(@"Data Source=.\SQLExpress;AttachDbFilename=E:\汪春晓\beneDesCYC\dataBase\MsgMass_Data.MDF;Integrated Security=True;User Instance=True;");
        ...
        internal string getYgsUserId()
        {
            string strconn = ConfigurationManager.ConnectionStrings["ygsConnectionString"].ConnectionString;
            string[] strs = strconn.Split(';');
            string UserID = string.Empty;
            foreach (string str in strs)
            {
                if (str.ToString().ToUpper().Trim().StartsWith("USER"))
                {
                    UserID = str;
                    break;
                }
            }
            string userid = UserID.Substring(UserID.IndexOf('=') + 1).Trim();
            return userid;

        }
        internal string getCycUserId()
        {
            string strconn = ConfigurationManager.ConnectionStrings["cycConnectionString"].ConnectionString;
            string[] strs = strconn.Split(';');
            string UserID = string.Empty;
        ...
        public OracleParameter[] PraseOracleNullField(OracleParameter[] parameters)
        {
            foreach (OracleParameter parmameter in parameters)
            {
                if (object.Equals(parmameter.Value, null) || object.Equals(parmameter.Value, string.Empty))
                {
                    parmameter.Value = DBNull.Value;
                }
            }
            return parameters;
        }
        public int ExcuteSql(string cmdText, CommandType cmdType, params OracleParameter[] paras)
        {
            int result = 0;
            OracleConnection conn = GetConn();
        ...
        public int ExecuteTranErrorCount(string cmdText, CommandType cmdType, params OracleParameter[] paras)
        {
            OracleConnection conn = GetConn();
            using (OracleConnection connection = conn)
            {
