Ext.onReady(function() {
    ...
    var events = {
        ...
        search: function() {
            ...
            document.getElementById('center').setAttribute('src', iframeUrl + '?startMonth=' + startMonth + '&endMonth=' + endMonth + '&CYC=' + cyc + '&lstartMonth=' + lstartMonth + '&lendMonth=' + lendMonth + '&dwType=' + dwType + '&zdw=' + zdw);
            // loading的实现
    ...
    var panel = Ext.create('Ext.panel.Panel', {
        ...
        //{ xtype: 'radio', boxLabel: '作业区', name: 'targetType', margin: '0 10 0 0', id: 'zyq' },
            ...
            '-',
            { xtype: 'combo', name: 'CYC', id: 'CYC', value: 'quan', store: departmentStore, labelWidth: 40, labelAlign: 'right', width: 90, displayField: 'DEP_NAME', valueField: 'DEP_ID', triggerAction: 'all'
                    , listeners: {
                        select: {
            ...
            '-',
            { xtype: 'combo', name: 'dwType', id: 'dwType', width: 70, displayField: 'DWNAME', valueField: 'DWID', triggerAction: 'all'
                , listeners: {
                    select: function(combo, record, index) {
                        ...
                        var zdwStore = Ext.create('Ext.data.Store', {
                            proxy: {
                                type: 'ajax',
                                url: '../../api/system/jdstat_djsj.aspx',
                                extraParams: {
                                    dwType: dwType,
                                    CYC: cyc,
                                    BNY: month,
            ...
            '-',
            { xtype: 'combo', name: 'zdw', id: 'zdw', hiddenName: 'zdw', width: 80, displayField: 'DEP_NAME', valueField: 'DEP_ID', queryMode: 'remote', triggerAction: 'all' },
            '-',
