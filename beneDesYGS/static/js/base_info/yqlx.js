Ext.onReady(function() {
    ...
    var events = {
        ...
        addClick: function() {
            isAdd = true;
            windowDialog.setTitle('添加油气类型');
            formPanel.getForm().reset();
            formPanel.getForm().findField('YQLXDM').setReadOnly(false);
            formPanel.getForm().findField('YQLXMC').setReadOnly(false);
        ...
        deleteClick: function() {
            ...
            Ext.Msg.confirm('确认', '您确定要删除该油气类型？', function(result) {
                if (result == 'yes') {
                    dataJs.get(
                        '../../api/base_info/yqlx.aspx',

                        {
                            _type: 'delete',
                            BZ: gridSel.data['BZ'],
                            YQLXDM: gridSel.data['YQLXDM'],
                            YQLXMC: gridSel.data['YQLXMC']
                        },
    ...
    */
    ...
    var gridPanel = Ext.create('Ext.grid.Panel', {
        ...
        columns: [
            ...
            { text: '名称', dataIndex: 'YQLXMC', width: 400, fixed: true },
            { text: '备注', dataIndex: 'BZ', width: 300, fixed: true },
