Ext.onReady(function() {
    ...
    var events = {
        ...
        addClick: function() {
            isAdd = true;
            windowDialog.setTitle('添加负效类型');
            formPanel.getForm().reset();
            formPanel.getForm().findField('FXYYDM').setReadOnly(false);
            formPanel.getForm().findField('FXYYMC').setReadOnly(false);
        ...
        deleteClick: function() {
            ...
            Ext.Msg.confirm('确认', '您确定要删除该负效类型？', function(result) {
                if (result == 'yes') {
                    dataJs.get(
                        '../../api/base_info/fxlx.aspx',

                        {
                            _type: 'delete',
                            BZ: gridSel.data['BZ'],
                            FXYYDM: gridSel.data['FXYYDM'],
                            FXYYMC: gridSel.data['FXYYMC']
                        },
        ...
        editClick: function() {
            ...
            formPanel.getForm().findField('BZ').setReadOnly(false);
            formPanel.getForm().findField('FXYYDM').setReadOnly(true);
            formPanel.getForm().findField('FXYYMC').setReadOnly(false);
    ...
    */
    ...
    var gridPanel = Ext.create('Ext.grid.Panel', {
        ...
        columns: [
            { text: '代码', dataIndex: 'FXYYDM' },
            { text: '名称', dataIndex: 'FXYYMC', width: 400, fixed: true },
