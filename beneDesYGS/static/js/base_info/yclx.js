Ext.onReady(function() {
    ...
    var events = {
        ...
        addClick: function() {
            isAdd = true;
            windowDialog.setTitle('添加油藏类型');
            formPanel.getForm().reset();
            formPanel.getForm().findField('YCLX').setReadOnly(false);
            //formPanel.getForm().findField('DZCSMC').setReadOnly(false);
        ...
        deleteClick: function() {
            ...
            Ext.Msg.confirm('确认', '您确定要删除该油藏类型？', function(result) {
                if (result == 'yes') {
                    dataJs.get(
                        '../../api/base_info/yclx.aspx',

                        {
                            _type: 'delete',
                            YCLX: gridSel.data['YCLX']
                            //DZCSDM: gridSel.data['DZCSDM'],
                            //DZCSMC: gridSel.data['DZCSMC']
                        },
        ...
        editClick: function() {
            ...
            formPanel.getForm().findField('YCLX').setReadOnly(false);
            //formPanel.getForm().findField('DZCSDM').setReadOnly(true);
            //formPanel.getForm().findField('DZCSMC').setReadOnly(false);
        ...
        addOrEdit: function() {
            ...
            if (form.isValid()) {
                ...
                if (!isAdd) {
                    values['_type'] = 'edit';
                    values['YCLX_old'] = gridSel.data['YCLX']
                }
    ...
    */
    ...
    var gridPanel = Ext.create('Ext.grid.Panel', {
        ...
        columns: [
        //{ text: ' ', dataIndex: 'DZCSDM' },
            { text: '油藏类型', dataIndex: 'YCLX',width:400,fixed:true }
            //{ text: ' ', dataIndex: 'BZ' },
